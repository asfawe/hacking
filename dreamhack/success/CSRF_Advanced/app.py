...
def check_csrf(param, cookie={"name": "name", "value": "value"}):
	...
	# 여기서 http://host3.dreamhack.games:20033/대신 http://127.0.0.1:8000/을 적는 이유는 우리가 실행할 flask 서버는 지금 웹에서 실행되고 있지 않고
	# localhost에서 실행이 되기 때문이다.
...
def change_password():
    ...
    if pw == None:
        ...
        if csrf_token != request.args.get("csrftoken", ""): # 우리가 굳이 안 넣어도 되는 이유는 이미 admin에는 csrftoken이 존재함....
